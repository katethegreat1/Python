...
# 两个提示：1.想清楚哪些代码要嵌套到for循环里，即一局战斗里包括什么信息。确定了for写在哪里之后，一句战斗包含的所有信息都要缩进；
# 2.细节也需要留意，如局与局之间要怎么区分开来（时间间隔&打印局数信息）

print('''如果做起来有些障碍，检查一下是否存在上面提示的这几个问题：1. for循环语句的位置放的对不对？这个关键在于，你想让哪些信息被循环展示。例如：如果你错将for循环语句放在了【随机属性】 和【自动战斗】之间，那每一局的战斗信息会是一样的，也就不存在什么三局两胜了。
